...
fn parse_macho(
    ...
    // Parse LC_DYLD_CHAINED_FIXUPS to extract rebase pointers.
    // preferred_base is the __TEXT segment's vmaddr (the image base), NOT the
    // __text section's address.  Fixup chain offsets are relative to the start
    // of the image file, which maps to preferred_base in the virtual address space.
...
fn build_macho_fixup_pointers(
    ...
    // Find the LC_DYLD_CHAINED_FIXUPS load command.
    ...
    // dyld_chained_fixups_header (7 × u32):
    ...
    // dyld_chained_starts_in_image: seg_count, then per-segment offset array.
    ...
    for seg_idx in 0..seg_count {
        ...
        // Only handle DYLD_CHAINED_PTR_64_OFFSET (6) and DYLD_CHAINED_PTR_64 (2).
        // Format 6: target is an offset from the image base.
        // Format 2: target is an absolute virtual address (for non-PIE).
